    ...
    const renderUserVote = (rating) => {
        ...
        return (
            <div className="flex items-center gap-1 font-bold text-xs">
                {isUpvote ? (
                    <span className="flex items-center gap-1 text-emerald-500"><ArrowBigUp size={16} className="fill-emerald-500" /> Aprovado</span>
                ) : (
                    <span className="flex items-center gap-1 text-red-500"><ArrowBigDown size={16} className="fill-red-500" /> Reprovado</span>
                )}
            </div>
        );
    };
