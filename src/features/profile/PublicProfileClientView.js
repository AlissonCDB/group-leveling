...
import { Briefcase, CalendarPlus, ChevronLeft, User as UserIcon, MapPin, Lock, Star, X } from 'lucide-react';
...
export default function PublicProfileClientView({ userData, works, meetings }) {
    ...
    // 🔴 ESTADOS PARA CONTROLAR QUAIS DADOS O MODAL VAI MOSTRAR
    const [activeModal, setActiveModal] = useState(null); // 'works' | 'meetings' | null

    const validWorks = works.filter(w => w.avgRating > 0);
    const globalWorksAvg = validWorks.length > 0 
        ? validWorks.reduce((sum, w) => sum + w.avgRating, 0) / validWorks.length 
        : 0;

    const validMeetings = meetings.filter(m => m.avgRating > 0);
    const globalMeetingsAvg = validMeetings.length > 0 
        ? validMeetings.reduce((sum, m) => sum + m.avgRating, 0) / validMeetings.length 
        : 0;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                    ...
                    <div className="bg-gray-900 border border-amber-500/30 rounded-3xl p-8 flex flex-col items-center text-center shadow-[0_0_20px_rgba(245,158,11,0.05)] relative overflow-hidden">
                        ...
                        <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mb-6">
                            {works.length} {works.length === 1 ? 'Publicação' : 'Publicações'} no total
                        </p>

                        <div className="flex items-center gap-3 mb-8 bg-gray-950 py-3 px-6 rounded-2xl border border-gray-800">
                            <span className="text-4xl font-black text-amber-500">{globalWorksAvg > 0 ? globalWorksAvg.toFixed(1) : '-'}</span>
                            <div className="flex flex-col items-start justify-center">
                                <div className="flex gap-0.5 mb-1" title={`Média Geral: ${globalWorksAvg.toFixed(1)}`}>
                                    {[...Array(5)].map((_, i) => (
                                        <Star 
                                            key={i} 
                                            size={16} 
                                            className={globalWorksAvg > 0 && i < Math.round(globalWorksAvg) ? "text-yellow-500 fill-yellow-500" : "text-gray-800"} 
                                        />
                                    ))}
                                </div>
                                <span className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">Média Geral</span>
                            </div>
                        </div>

                        <button 
                            onClick={() => setActiveModal('works')}
                            className="w-full flex items-center justify-center gap-2 py-3 bg-amber-500/10 hover:bg-amber-500/20 text-amber-500 border border-amber-500/30 rounded-xl font-bold uppercase text-xs tracking-widest transition-all"
                        >
                            Visualizar Trabalhos Completos
                        </button>
                    </div>
                    ...
                    <div className="bg-gray-900 border border-blue-500/30 rounded-3xl p-8 flex flex-col items-center text-center shadow-[0_0_20px_rgba(59,130,246,0.05)] relative overflow-hidden">
                        ...
                        <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mb-6">
                            {meetings.length} {meetings.length === 1 ? 'Missão' : 'Missões'} no total
                        </p>

                        <div className="flex items-center gap-3 mb-8 bg-gray-950 py-3 px-6 rounded-2xl border border-gray-800">
                            <span className="text-4xl font-black text-blue-500">{globalMeetingsAvg > 0 ? globalMeetingsAvg.toFixed(1) : '-'}</span>
                            <div className="flex flex-col items-start justify-center">
                                <div className="flex gap-0.5 mb-1" title={`Média Geral: ${globalMeetingsAvg.toFixed(1)}`}>
                                    {[...Array(5)].map((_, i) => (
                                        <Star 
                                            key={i} 
                                            size={16} 
                                            className={globalMeetingsAvg > 0 && i < Math.round(globalMeetingsAvg) ? "text-yellow-500 fill-yellow-500" : "text-gray-800"} 
                                        />
                                    ))}
                                </div>
                                <span className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">Média Geral</span>
                        ...
                        <button 
                            ...
                            className="w-full flex items-center justify-center gap-2 py-3 bg-blue-500/10 hover:bg-blue-500/20 text-blue-500 border border-blue-500/30 rounded-xl font-bold uppercase text-xs tracking-widest transition-all"
                        >
                            Visualizar Missões Completas
                        </button>
                    </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-xl font-black text-white uppercase tracking-widest">
                                        {activeModal === 'works' ? 'Arquivo de Conhecimento' : 'Missões Lideradas'}
                                    </h2>
                                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">
                                        Histórico completo de {userData?.user_name}
                                    </p>
                                </div>
                            </div>
                            <button 
                        ...
                        <div className="p-6 overflow-y-auto scrollbar-hide flex-1 bg-gray-950/50 space-y-4">
                                    ...
                                    <div key={work.id} className="bg-gray-900 border border-gray-800 p-5 rounded-2xl hover:border-amber-500/30 transition-colors">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                {work.avgRating > 0 ? (
                                                    <div className="flex items-center gap-0.5 border-l border-gray-700 pl-3">
                                                        {[...Array(5)].map((_, i) => (
                                                            <Star 
                                                                key={i} 
                                                                size={14} 
                                                    ...
                                                    <div className="flex items-center border-l border-gray-700 pl-3">
                                                        <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">Sem Notas</span>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <span className="flex items-center gap-1 text-[10px] font-bold text-gray-600 uppercase tracking-widest cursor-not-allowed select-none">
                                                <Lock size={12} /> Arquivado
                                            </span>
                                        </div>
                                    </div>
                                )) : (
                                    <div className="text-center p-12 border border-dashed border-gray-800 rounded-2xl">
                                        <p className="text-gray-600 italic text-sm">Nenhum trabalho publicado.</p>
                                    </div>
                                )
                            )}

                            {/* SE FOR MODAL DE RAIDS */}
                            {activeModal === 'meetings' && (
                                meetings.length > 0 ? meetings.map(raid => (
                                    ...
                                    <div className="text-center p-12 border border-dashed border-gray-800 rounded-2xl">
                                        <p className="text-gray-600 italic text-sm">Nenhuma raid organizada.</p>
                                    </div>
                                )
                            )}
                        </div>
