...
import { Trophy, Medal, Award, Briefcase, CalendarPlus, X, ArrowBigUp, ArrowBigDown } from 'lucide-react'; 
...
export default function RanksClientView({ allUsersData, currentUserId }) {
    ...
    const worksRanking = useMemo(() => {
        ...
        return allUsersData.map(user => {
            ...
            validWorks.forEach(work => {
                ...
                evaluations.forEach(uw => {
                    if (uw && uw.rating !== null && uw.rating !== undefined) {
                        totalScore += Number(uw.rating); 
                        ratingCount++;
                    }
                });
            });
        ...
        .filter(user => viewMode === 'quantidade' ? user.score > 0 : user.hasRatings)
        .sort((a, b) => {
            if (viewMode === 'quantidade') {
                return b.score - a.score || b.totalScore - a.totalScore;
            } else {
                return b.totalScore - a.totalScore || b.score - a.score;
            }
        }); 
        ...
        return allUsersData.map(user => {
            ...
            validMeetings.forEach(raid => {
                ...
                evaluations.forEach(um => {
                    if (um && um.rating !== null && um.rating !== undefined) {
                        totalScore += Number(um.rating); 
                        ratingCount++;
                    }
                });
            });
        ...
        return (
            <div className="flex flex-col gap-3">
                <div className="hidden md:flex px-6 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                    ...
                    <div className="w-32 text-right">{viewMode === 'quantidade' ? labelScore : 'Pontos'}</div>
                </div>
                        ...
                        <div key={user.id} onClick={() => handleViewProfile(user.id)} className={`flex items-center p-4 rounded-xl border ${rankStyle.border} ${rankStyle.bg} hover:bg-gray-800 transition-all cursor-pointer relative overflow-hidden group`}>
                            ...
                            <div className="w-32 text-right">
                                {viewMode === 'quantidade' ? (
                                    ...
                                    <div className={`text-sm font-black flex items-center justify-end gap-1 ${user.totalScore >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                                        {user.totalScore > 0 ? `+${user.totalScore}` : user.totalScore} 
                                        {user.totalScore >= 0 ? <ArrowBigUp size={16} className="fill-emerald-500" /> : <ArrowBigDown size={16} className="fill-red-500" />}
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 mb-6 border-b border-gray-800 pb-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-500 uppercase tracking-widest">
                                    {viewMode === 'quantidade' ? 'Baseado em Trabalhos Publicados' : 'Baseado na Pontuação de Trabalhos'}
                                </p>
                            </div>
                        </div>
                    ...
                    <section>
                        <div className="flex items-center gap-3 mb-6 border-b border-gray-800 pb-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-500 uppercase tracking-widest">
                                    {viewMode === 'quantidade' ? 'Baseado em Raids Agendadas' : 'Baseado na Pontuação de Raids'}
                                </p>
                            </div>
                        </div>
                                    ...
                                    <div className="flex items-center p-3 rounded-xl border border-amber-500/50 bg-amber-500/10 shadow-[0_0_15px_rgba(245,158,11,0.1)]">
                                        ...
                                        <div className="w-32 text-right">
                                            {viewMode === 'quantidade' ? (
                                                ...
                                                <div className={`text-sm font-black flex items-center justify-end gap-1 ${myRankData.totalScore >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                                                    {myRankData.totalScore > 0 ? `+${myRankData.totalScore}` : myRankData.totalScore} 
                                                    {myRankData.totalScore >= 0 ? <ArrowBigUp size={16} className="fill-emerald-500" /> : <ArrowBigDown size={16} className="fill-red-500" />}
                                                </div>
                                            )}
                                        </div>
                                    </div>
