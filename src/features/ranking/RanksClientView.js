    ...
    const worksRanking = useMemo(() => {
        ...
        .sort((a, b) => {
            if (viewMode === 'quantidade') {
                return b.score - a.score || b.avgRating - a.avgRating;
            } else {
                return b.avgRating - a.avgRating || b.score - a.score;
            }
        }); 
        ...
        return (
            <div className="flex flex-col gap-3">
                <div className="hidden md:flex px-6 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                    ...
                    <div className="w-40 text-right">{viewMode === 'quantidade' ? labelScore : 'Média'}</div>
                </div>
                        ...
                        <div key={user.id} onClick={() => handleViewProfile(user.id)} className={`flex items-center p-4 rounded-xl border ${rankStyle.border} ${rankStyle.bg} hover:bg-gray-800 transition-all cursor-pointer relative overflow-hidden group`}>
                            ...
                            <div className="w-40 text-right">
                                {viewMode === 'quantidade' ? (
                                    ...
                                    <div className="flex items-center justify-end gap-1" title={`${user.avgRating} Estrelas`}>
                                        <span className="text-xs font-bold text-amber-500 mr-1">{user.avgRating}</span>
                                        {[...Array(5)].map((_, i) => (
                                            <Star 
                                                key={i} 
                                                size={14} 
                                                // Arredondamento visual: 4.5 vira 5 estrelas acesas
                                                className={i < Math.round(user.avgRating) ? "text-yellow-500 fill-yellow-500" : "text-gray-700"} 
                                            />
                                        ))}
                                    </div>
                                )}
                                    ...
                                    <div className="flex items-center p-3 rounded-xl border border-amber-500/50 bg-amber-500/10 shadow-[0_0_15px_rgba(245,158,11,0.1)]">
                                        ...
                                        <div className="w-40 text-right">
                                            {viewMode === 'quantidade' ? (
                                                ...
                                                <div className="flex items-center justify-end gap-1" title={`${myRankData.avgRating} Estrelas`}>
                                                    <span className="text-xs font-bold text-amber-500 mr-1">{myRankData.avgRating}</span>
                                                    {[...Array(5)].map((_, i) => (
                                                        <Star 
                                                            key={i} 
                                                            size={14} 
                                                            className={i < Math.round(myRankData.avgRating) ? "text-yellow-500 fill-yellow-500" : "text-gray-700"} 
                                                        />
                                                    ))}
                                                </div>
                                            )}
